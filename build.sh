...
if [[ ! "$rounds" =~ ^[1-9][0-9]*$ ]]; then
    printf 'Usage: ./build.sh [positive number of runs]\n' >&2
    exit 2
fi

if ! command -v javac >/dev/null || ! command -v java >/dev/null; then
    printf 'JDK 17 or newer is required: both javac and java must be available.\n' >&2
    exit 1
fi
...
# javac argument files avoid the need for Maven, Gradle, or dependencies.
